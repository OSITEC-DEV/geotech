...
class CustomerPortalTest(CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'report_count' in counters:
            values['report_count'] = MedicalReport.search_count(self._prepare_orders_test_domain(partner)) \
                if MedicalReport.check_access_rights('read', raise_exception=False) else 0
        return values

    @http.route(['/my/reports/<model("lims.report"):report_id>'], type='http', auth="user", website=True)
    def portal_report_form_page(self, report_id, report_type=None, message=False, access_token=None, **kw):
        if report_type == 'html':
            ...
            vals = {'report': report_sudo.sudo(), 'page_name': 'medical_report_form', 'message': message}
    ...
    # def portal_report_pdf_page(self, report_id,report_type=None, **kw):
    #     if report_type == 'pdf'
    #         vals = {'report': report_id, 'page_name': 'medical_report_form', }
    #         return request.render('lims_report.medical_analysis_report_form_view_portal', vals)

    def _prepare_orders_test_domain(self, partner):
        return [
    ...
    def portal_my_reports_by_test(self, page=1, date_begin=None, date_end=None, search="", groupby='none',
                                  search_in="All", sortby=None, **kw):
        values = self._prepare_portal_layout_values()
        partner = request.env.user.partner_id
        MedicalReport = request.env['lims.report'].sudo()

        domain = self._prepare_orders_test_domain(partner)
        if not groupby:
            groupby = "none"
        searchbar_sortings = {
        ...
        request.session['my_reports_test_history'] = reports.ids[:100]
        ...
        return request.render("lims_report.portal_my_reports_by_test", values)

    @http.route(['/my/reports/<model("lims.report"):report_id>/accept'], type='json', auth="user", website=True)
    def portal_route_accept(self, report_id, access_token=None, name=None, signature=None, **post):
